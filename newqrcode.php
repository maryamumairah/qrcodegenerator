...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!preg_match($validPrefixPattern, $startPrefix) || !preg_match($validPrefixPattern, $endPrefix)) {
        $_SESSION['error'] = "Error: Invalid prefix format. Prefix must be A-Z only.";
    ...
    if (!in_array($startRunningChar, $validChars) || !in_array($endRunningChar, $validChars)) {
        $_SESSION['error'] = "Error: Invalid running character format. Running character must be A-Z except 'I' and 'O'.";
        header("Location: newform.php");
        exit();
    }

    if (!preg_match('/^\d{1,7}$/', $_POST['startNumber']) || intval($_POST['startNumber']) > 9999999 || intval($_POST['startNumber']) === 0) {
        $_SESSION['error'] = "Error: Invalid start running number format. Running number must be between 1 and 9999999.";
        header("Location: newform.php");
        exit();
    }

    if (!preg_match('/^\d{1,7}$/', $_POST['endNumber']) || intval($_POST['endNumber']) > 9999999 || intval($_POST['endNumber']) === 0) {
        $_SESSION['error'] = "Error: Invalid end running number format. Running number must be between 1 and 9999999.";
        header("Location: newform.php");
        exit();
    }
    ...
    if (!preg_match($validPostfixPattern, $startPostfix) || !preg_match($validPostfixPattern, $endPostfix)) {
        $_SESSION['error'] = "Error: Invalid postfix format. Postfix must be A-Z only.";
        header("Location: newform.php");
        exit();
    }
    ...
    if ($productType === "Electrical") {
        $csvHeaders = ["SIRIM SERIAL NO.", "BATCH NO.", "MANUFACTURER DATE", "BRAND/TRADEMARK", "MODEL", "TYPE", "RATING", "SIZE", "COA NUMBER"];
    } else {
    ...
    $localIp = "10.255.7.27"; // changed 18/4
    // $localIp = $_SERVER['SERVER_ADDR']; // changed 21/3
    $csvUrl = "http://" . $localIp . "/qrcodegenerator/generated_csv/" . $filename;
    
    // Hides /qrcodegenerator directory from URL ❌ not working
    // $csvUrl = "http://" . $localIp . "/generated_csv/" . $filename;
    ...
    //         VALUES (?, ?, ?, ?, ?)";
    $sql = "INSERT INTO `transaction_log` (product_type, start_serial, end_serial, excluded_serials, user, qr_code_image) 
            VALUES (?, ?, ?, ?, ?, ?)";
    
    $stmt = $con->prepare($sql);
    // $stmt->bind_param("sssss", $startSerial, $endSerial, $excludedSerialsString, $userPC, $qrImageData);
    $stmt->bind_param("ssssss", $productType, $startSerial, $endSerial, $excludedSerialsString, $userPC, $qrImageData);
...
<body>
    <div class="output-container">
        <?php if (!empty($csvUrl) && !empty($qrCodeUrl)): ?>
            <h3>Scan the QR code to access the CSV file:</h3>
            <img src="<?= $qrCodeUrl ?>" alt="QR Code" class="qr-code">
            <h3><a href="<?= $csvUrl ?>" download>Download CSV</a></h3>
            <h4>Generated by: <?= htmlspecialchars($userPC) ?></h4>
        <?php endif; ?>
        <div class="btn-container">
            ...
            <button class="btn view-report-btn">
                ...
                <a href="/qrcodegenerator/newtransaction_log.php" target="_blank">View Transaction Report</a>
            </button>
            <form action="newform.php" method="get">
